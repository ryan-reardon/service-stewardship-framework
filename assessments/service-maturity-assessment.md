# Service Maturity Assessment Questionnaire
## Enterprise Technology Service Stewardship Framework

This consolidated assessment evaluates maturity across all 12 domains.

Scoring Scale:
1 = Reactive / Informal  
2 = Managed / Documented but inconsistent  
3 = Optimized / Measured and governed  
4 = Predictive / Proactive and continuously improving  

Evidence should be cited for each score.

Related references:
- [Framework Definition](../docs/framework.md)
- [Maturity Model](../maturity-model/README.md)
- [Scoring Guidance](../maturity-model/scoring-guidance.md)
- [How to Use the Framework](../docs/how-to-use.md)
- [Per-Domain Assessment Modules](modules/)

---

## 1. Strategic Stewardship
- Documented multi-quarter roadmap aligned to enterprise strategy
- Initiatives tied to measurable business outcomes
- Lifecycle stage explicitly defined
- Modernization capacity protected
- Cost-to-value trend reviewed quarterly

Detail: [Assessment Module](modules/01-strategic-stewardship.md) · [Domain Module](../modules/01-strategic-stewardship.md) · [Playbook](../playbooks/01-strategic-stewardship.md)

## 2. Organizational Capability
- Documented team charter and ownership boundaries
- Skills matrix aligned to roadmap and ops needs
- Redundancy for critical responsibilities
- Sustainable on-call workload
- Active growth and succession planning

Detail: [Assessment Module](modules/02-organizational-capability.md) · [Domain Module](../modules/02-organizational-capability.md) · [Playbook](../playbooks/02-organizational-capability.md)

## 3. Value Execution
- Explicit value-based prioritization
- Predictable delivery cadence
- NFRs included in definition of done
- Dependencies actively managed
- Post-delivery outcome measurement

Detail: [Assessment Module](modules/03-value-execution.md) · [Domain Module](../modules/03-value-execution.md) · [Playbook](../playbooks/03-value-execution.md)

## 4. Engineering Excellence
- CI/CD with enforced quality gates
- Infrastructure as code
- Change failure rate tracked
- Toil measured and reduced
- Secure SDLC embedded

Detail: [Assessment Module](modules/04-engineering-excellence.md) · [Domain Module](../modules/04-engineering-excellence.md) · [Playbook](../playbooks/04-engineering-excellence.md)

## 5. Operational Reliability
- SLAs/SLOs defined and measured
- Error budgets influence change decisions
- Postmortem actions tracked
- Alert noise actively reduced
- Capacity planning proactive

Detail: [Assessment Module](modules/05-operational-reliability-assessment.md) · [Domain Module](../modules/05-operational-reliability.md) · [Playbook](../playbooks/05-operational-reliability.md)

## 6. Architectural Integrity
- Architecture documented and current
- ADRs used
- Architectural debt tracked
- NFRs evaluated in reviews
- Tooling sprawl governed

Detail: [Assessment Module](modules/06-architectural-integrity-assessment.md) · [Domain Module](../modules/06-architectural-integrity.md) · [Playbook](../playbooks/06-architectural-integrity.md)

## 7. Service Experience
- Support model documented
- Response/resolution targets defined
- Repeat issues reduced
- Knowledge base maintained
- Customer feedback influences roadmap

Detail: [Assessment Module](modules/07-service-experience-assessment.md) · [Domain Module](../modules/07-service-experience.md) · [Playbook](../playbooks/07-service-experience.md)

## 8. Security & Risk Governance
- Complete dependency inventory
- Vulnerability SLAs enforced
- Patch cadence consistent
- Access reviews conducted
- Compliance evidence automated

Detail: [Assessment Module](modules/08-security-risk-governance-assessment.md) · [Domain Module](../modules/08-security-risk-governance.md) · [Playbook](../playbooks/08-security-risk-governance.md)

## 9. Financial Stewardship
- Full TCO visibility
- Unit cost tracked
- Forecast updated quarterly
- Cost optimization backlog maintained
- Vendor/license governance active

Detail: [Assessment Module](modules/09-financial-stewardship-assessment.md) · [Domain Module](../modules/09-financial-stewardship.md) · [Playbook](../playbooks/09-financial-stewardship.md)

## 10. Observability & Intelligence
- SLOs instrumented and reliable
- Alert quality high
- Tracing for critical flows
- Telemetry cost governed
- Forecasting based on telemetry trends

Detail: [Assessment Module](modules/10-observability-intelligence-assessment.md) · [Domain Module](../modules/10-observability-intelligence.md) · [Playbook](../playbooks/10-observability-intelligence.md)

## 11. Innovation & Modernization
- Technical debt categorized and scored
- Modernization capacity protected
- Legacy components tracked
- Experiments structured and measured
- Modernization outcomes quantified

Detail: [Assessment Module](modules/11-innovation-modernization-assessment.md) · [Domain Module](../modules/11-innovation-modernization.md) · [Playbook](../playbooks/11-innovation-modernization.md)

## 12. Portfolio Alignment
- Dependency map current
- Integration contracts versioned
- Architecture compliance governed
- Duplication reviewed
- Cross-service risks tracked

Detail: [Assessment Module](modules/12-portfolio-alignment-assessment.md) · [Domain Module](../modules/12-portfolio-alignment.md) · [Playbook](../playbooks/12-portfolio-alignment.md)

---

Overall Score: ______  
Priority Domains: ______
